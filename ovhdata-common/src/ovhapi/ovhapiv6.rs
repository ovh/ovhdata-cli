...
impl OVHapiV6Client {
    ...
    pub fn new(endpoint_url: String, application_key: String, application_secret: String, consumer_key: String) -> Self {
        Self {
            endpoint_url,
            application_key,
            application_secret,
    ...
    fn build_sig(method: &str, query: &str, body: &str, timestamp: &str, aas: &str, ck: &str) -> String {
        let sep = "+";
        let prefix = "$1$".to_string();

        let capacity =
            1 + aas.len() + sep.len() + ck.len() + method.len() + sep.len() + query.len() + sep.len() + body.len() + sep.len() + timestamp.len();
    ...
    async fn remote_time(&self) -> Result<u64> {
        let request = self
            .build_request_without_authent(Method::GET, &["auth", "time"], &[], &HeaderMap::new(), EMPTY_BODY)
            .await?;
