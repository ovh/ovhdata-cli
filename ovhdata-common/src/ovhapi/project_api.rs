...
pub trait ProjectApi {
    ...
    async fn project(&self, service_name: &String) -> Result<Project>;
...
impl ProjectApi for OVHapiV6Client {
    ...
    async fn project(&self, service_name: &String) -> Result<Project> {
        let request = self.build_request(Method::GET, &["cloud", "project", service_name.as_str()], &[], &HeaderMap::new(), EMPTY_BODY).await?;
        let response = request.send(&self.client, &[]).await?;
        response.parse().await
    }

    async fn project_list(&self) -> Result<Vec<String>> {
        let request = self.build_request(Method::GET, &["cloud", "project"], &[], &HeaderMap::new(), EMPTY_BODY).await?;
        let response = request.send(&self.client, &[]).await?;
        response.parse().await
    }
}