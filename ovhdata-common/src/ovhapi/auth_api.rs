...
impl AuthApi for OVHapiV6Client {
    async fn me(&self) -> Result<Me> {
        let request = self.build_request(Method::GET, &["auth", "details"], &[], &HeaderMap::new(), EMPTY_BODY).await?;
        let response = request.send(&self.client, &[]).await?;
        response.parse().await
    }

    async fn current_credential(&self) -> Result<CredentialDetails> {
        let request = self.build_request(Method::GET, &["auth", "currentCredential"], &[], &HeaderMap::new(), EMPTY_BODY).await?;
        let response = request.send(&self.client, &[]).await?;
        response.parse().await
    }
}