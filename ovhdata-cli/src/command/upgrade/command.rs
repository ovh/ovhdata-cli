...
use crate::utils::ui::printer::{stderr, Printer, HELP_UPGRADE};
...
impl Upgrade {
    ...
    pub async fn upgrade_os(&self, quiet: bool, _last_version: &Version) -> Result<()> {
        if !quiet {
            eprintln!("Download the new version of the CLI at, auto-upgrade is not available for Windows yet:");
            eprintln!("{}", Self::get_binary_url());
            Printer::eprintln_fail("Only the last version of ovhdata-cli is supported");
        }
        Ok(())
    }
    ...
    async fn get_binary_url() -> Result<String> {
        ...
        // Split assets into kv
        let github_assets : HashMap<String, String> = if github_release.assets.is_empty() {
            HashMap::new()
        } else {
            github_release.assets
                .iter()
                .map(|asset| (asset.clone().label, asset.clone().browser_download_url))
                .collect::<HashMap<_, _>>()
        };

        // Return the download url corresponding to the OS
        let url = github_assets.get(OS).cloned();
        url.ok_or(Error::Custom(format!("Unknown OS {}", OS)))
    }
