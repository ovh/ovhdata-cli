...
impl Auth {
    ...
    pub async fn login(&self, application_key: Option<String>, consumer_key: Option<String>, application_secret: Option<String>, output: Output) -> Result<()> {
        ...
        let creds = if interactive {
            // If the config exists, test it
            {
                ...
                if context.ovhapi_credentials.is_some() {
                        ...
                        if let api::Error::Response(status_code, message) = error {
                            ...
                            match status_code {
                                reqwest::StatusCode::FORBIDDEN | reqwest::StatusCode::UNAUTHORIZED => {
                                    ...
                                    Printer::eprintln_fail(&format!("{}", message));
                                },
                    ...
                    if confirm.is_err() {
                        return Err(Error::Custom(format!("Maybe another day ;-)")));
                    }
                }
            }
