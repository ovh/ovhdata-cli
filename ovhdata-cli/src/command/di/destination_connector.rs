...
use crate::options::{DestConnectorGet, DiSubDestConnectorCommands};
...
impl DestinationConnectorCommand {
    ...
    pub async fn execute_command(
        &self,
        connector_commands: DiSubDestConnectorCommands,
    ) -> Result<()> {
        match connector_commands {
            DiSubDestConnectorCommands::List(connector_list) => {
                self.list_destination_connectors(connector_list.output.unwrap_or_default().into())
                    .await
            }
            DiSubDestConnectorCommands::Get(connector_get) => {
                self.get_destination_connector(
                    &connector_get,
    ...
    async fn get_destination_connector(
        &self,
        input: &DestConnectorGet,
        output: Output,
    ) -> Result<()> {
        let service_name = Context::get().get_current_service_name().unwrap();
        ...
        if interactive {
            Printer::print_command(&format!(
                "di destination-connector get {} --service-name {} ",
                &id, &service_name
            ));
        }
        ...
        let connector = self
            .rcp_client
            .clone()
            .di_destination_connector(&service_name, &id)
            .await?;
